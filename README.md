...
| `Log reading session` | Update reading progress, choose the session date and time, capture optional minutes spent, append to `reading-log.md`, append a timestamped line to that date’s daily note, and add the same session to the book note `## Log` section |
